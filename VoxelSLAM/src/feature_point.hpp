...
public:
...
#endif
  {
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      added_pt.curvature = (pl_orig.points[i].timestamp - time_head);        // s
      // added_pt.curvature = (pl_orig.points[i].timestamp - time_head) * 1e-3; // ms
      // added_pt.curvature = (pl_orig.points[i].timestamp - time_head) * 1e-6; // us
      // added_pt.curvature = (pl_orig.points[i].timestamp - time_head) * 1e-9; // ns
